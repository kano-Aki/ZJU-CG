...
int main()
{
    ...
    ShaderProgram shaderProgram("D:/ZJU/learn/31/cg/ZJU-CG/src/lab3/shader/solar.vs","D:/ZJU/learn/31/cg/ZJU-CG/src/lab3/shader/solar.fs");//创建着色器程序
    ...
    unsigned char *data=stbi_load("D:/ZJU/learn/31/cg/ZJU-CG/src/lab3/image/sun.jpg",&image_width,&image_height,&image_nrChannels,0);
    if(data)
    {
    ...
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR_MIPMAP_LINEAR);//设置纹理过滤方式
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_LINEAR);//设置纹理过滤方式
    data=stbi_load("D:/ZJU/learn/31/cg/ZJU-CG/src/lab3/image/earth.jpg",&image_width,&image_height,&image_nrChannels,0);
    if(data)
    {
        glTexImage2D(GL_TEXTURE_2D,0,GL_RGB,image_width,image_height,0,GL_RGB,GL_UNSIGNED_BYTE,data);//生成纹理
        glGenerateMipmap(GL_TEXTURE_2D);//生成多级渐远纹理
    ...
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR_MIPMAP_LINEAR);//设置纹理过滤方式
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_LINEAR);//设置纹理过滤方式
    data=stbi_load("D:/ZJU/learn/31/cg/ZJU-CG/src/lab3/image/saturn.jpg",&image_width,&image_height,&image_nrChannels,0);
    if(data)
    {
        glTexImage2D(GL_TEXTURE_2D,0,GL_RGB,image_width,image_height,0,GL_RGB,GL_UNSIGNED_BYTE,data);//生成纹理
        glGenerateMipmap(GL_TEXTURE_2D);//生成多级渐远纹理
